//CURRENTLY each time the program is run, the distances and times seem to be slightly different
...
int main(int argc, char **argv) {
	...
	//additonal code to stdr_open_loop_commander

	//TURN RIGHT
	twist_cmd.linear.x=0.0;
	twist_cmd.angular.z=-yaw_rate; //start spinning in place to the right
	...
	//TURN LEFT
	twist_cmd.linear.x=0.0; //stop moving forward
	twist_cmd.angular.z=yaw_rate; //start spinning in place to the left
	timer = 0.0; //reset timer
	while (timer < time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
